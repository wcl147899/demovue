        ...
        <el-form :model="form" label-width="120px">
          ...
          <el-form-item label="所属医院">
           ...
           <el-form-item label="类型">
            <el-select clearable filterable class="filter-item" style="width: 80%" placeholder="请选择所属院区" v-model="form.type">
              <el-option v-for="item in typeOptions3" :key="item.key" :label="item.display_name+'('+item.key+')'" :value="item.key"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="位置">
            <el-select clearable filterable class="filter-item" style="width: 80%" placeholder="请选择所属院区" v-model="form.location">
              <el-option v-for="item in typeOptions4" :key="item.key" :label="item.display_name+'('+item.key+')'" :value="item.key"></el-option>
            </el-select>
          </el-form-item>
        </el-form>
        <template #footer>
      ...
      <el-dialog title="提示" :visible.sync="dialogFormVisible" width="30%">
        <el-form :model="form" label-width="120px">
          ...
          <el-form-item label="英文名">
           ...
           <el-form-item label="类型">
            <el-select clearable filterable class="filter-item" style="width: 80%" placeholder="请选择类型" v-model="form.type">
              <el-option v-for="item in typeOptions3" :key="item.key" :label="item.display_name+'('+item.key+')'" :value="item.key"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="位置">
            <el-select clearable filterable class="filter-item" style="width: 80%" placeholder="请选择位置" v-model="form.location">
              <el-option v-for="item in typeOptions4" :key="item.key" :label="item.display_name+'('+item.key+')'" :value="item.key"></el-option>
            </el-select>
          </el-form-item>
        </el-form>
...
export default {
  ...
  methods:{
    ...
    save(){
          ...
          request.put("/device_put",this.form).then(res => {
          console.log(res)
          if (res.code ==='0'){
    ...
    getDropdown1(){
      request.get("/device_dropdown1").then(res => {
          console.log("我在下拉框");
          console.log(res);
     ...
     getDropdown2(){
      request.get("/device_dropdown2").then(res => {
          console.log("我在下拉框");
          console.log(res);
    ...
    getDropdown3(){
      request.get("/device_dropdown3").then(res => {
          console.log("我在下拉框");
          console.log(res);
          this.typeOptions3=res.data.dropdown;
      })
    },
    getDropdown4(){
      request.get("/device_dropdown4").then(res => {
          console.log("我在下拉框");
          console.log(res);
          this.typeOptions4=res.data.dropdown;
      })
    },

    submit(){
      request.post("/device_save",this.form)
      .then(res => {
          let vm=this;
          console.log(res)
          if (res.code ==='0'){
    ...
    handleDelete(id){
      ...
      request.delete("/device_delete/"+id).then(res =>{//注意delete请求的参数要用+号拼接到路径上，和后台代码是一致的
        let vm=this;
        if (res.code ==='0'){
